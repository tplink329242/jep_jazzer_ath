...
public class TestSharedInterpreter extends Thread {
    ...
    //     testSharedTypes();
    // }
    public static void test(int values) throws Throwable {
        testSharedModule(values);
        testSharedJep();
        testSharedTypes();
    }

    public static void testSharedModule(int values) throws Throwable{

        Random seedRandom1 = new Random(values);

        //max thread num
        int bound = 1024;
        int value = seedRandom1.nextInt(bound);

        TestSharedInterpreter[] t = new TestSharedInterpreter[value];
